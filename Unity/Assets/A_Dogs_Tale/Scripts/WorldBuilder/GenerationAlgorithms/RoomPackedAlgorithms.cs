...
public partial class DungeonGenerator : MonoBehaviour
{
    ...
    {
        if (cfg == null || !cfg.IsPackedParkTheme() || rooms == null)
            return;
    ...
    IEnumerator EnsurePackedRoomsConnectToCorridor(int yieldEvery = 64)
    {
        ...
        var finalCandidates = CollectDoorCandidates(W, H, cfg.GetEffectivePackedWallMoat(), cfg.doors.minDoorSpacing, yieldEvery);
    ...
    */
    ...
    int SplitOversizedRooms(int moatCells, List<HashSet<(int, int)>> frontiers)
    {
        ...
        for (int i = 0; i < rooms.Count; i++)
        {
            ...
            for (int old_cell_num = 0; old_cell_num < r.cells.Count; old_cell_num++)
            {
                ...
                if (cfg.UseThinWallsEffective())   // thin walls => no cut, only left/right
                {
    ...
    void AssignRoomUses()
    {
        foreach (Room room in rooms)
        {
            ...
            ApplyRoomEnvironment(room);
            ApplyRoomPresentation(room);
    ...
    void AssignRandomRoomUse(Room room)
    {
        ...
        room.placementTypes = PickWeightedRandom(cfg.GetActivePackedRoomTypeWeights());
    }

    void ApplyRoomEnvironment(Room room)
    {
        if (room == null)
            return;
    ...
    void ApplyRoomPresentation(Room room)
    {
        if (room == null)
            return;
    ...
    PlacementRoomTypeFlags PickWeightedRandom(DungeonSettings.RoomTypeWeight[] entries)
    {
